...
export function swapHundredsAndTens(number) {
    let sot = Math.floor(number / 100); // получаем цифру сотен
    let dec = Math.floor((number % 100) / 10); // получаем цифру десятков
    let ex = number % 10; // получаем цифру единиц
  
    let swappedNumber = ex * 100 + dec * 10 + sot; // переставляем цифры
...
export function countSquares(a ,b ,c) {
    let ac = Math.floor(a / c);
    let bc = Math.floor(b / c);
    return (ac * bc)
}
